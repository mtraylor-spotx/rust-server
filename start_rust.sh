...
exit_handler()
{
	...
	if [ -f "/home/rust/steamcmd/rust/seed_override" ]; then
		# Create the backup directory if it doesn't exist
		if [ ! -d "/home/rust/steamcmd/rust/bak" ]; then
			mkdir -p /home/rust/steamcmd/rust/bak
		fi
		if [ -f "/home/rust/steamcmd/rust/server/$RUST_SERVER_IDENTITY/UserPersistence.db" ]; then
			# Backup all the current unlocked blueprint data
			cp -fr "/home/rust/steamcmd/rust/server/$RUST_SERVER_IDENTITY/UserPersistence*.db" "/home/rust/steamcmd/rust/bak/"
		fi

		if [ -f "/home/rust/steamcmd/rust/server/$RUST_SERVER_IDENTITY/xp.db" ]; then
			# Backup all the current XP data
			cp -fr "/home/rust/steamcmd/rust/server/$RUST_SERVER_IDENTITY/xp*.db" "/home/rust/steamcmd/rust/bak/"
		fi
	fi
	
	# Execute the RCON shutdown command
	node /home/rust/shutdown_app/app.js
	sleep 5
...
if [ ! -d "/home/rust/steamcmd/rust" ]; then
	echo "Creating folder structure.."
	mkdir -p /home/rust/steamcmd/rust
fi

# Install/update steamcmd
echo "Installing/updating steamcmd.."
curl -s http://media.steampowered.com/installer/steamcmd_linux.tar.gz | tar -v -C /home/rust/steamcmd -zx

# Check which branch to use
if [ ! -z ${RUST_BRANCH+x} ]; then
	echo "Using branch arguments: $RUST_BRANCH"
	sed -i "s/app_update 258550.*validate/app_update 258550 $RUST_BRANCH validate/g" /home/rust/install.txt
else
	sed -i "s/app_update 258550.*validate/app_update 258550 validate/g" /home/rust/install.txt
fi

# Disable auto-update if start mode is 2
if [ "$RUST_START_MODE" = "2" ]; then
	# Check that Rust exists in the first place
	if [ ! -f "/home/rust/steamcmd/rust/RustDedicated" ]; then
		# Install Rust from install.txt
		echo "Installing Rust.. (this might take a while, be patient)"
		bash /home/rust/steamcmd/steamcmd.sh +runscript /home/rust/install.txt
...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/home/rust/steamcmd/rust/RustDedicated_Data/Plugins/x86_64

# Check if Oxide is enabled
if [ "$RUST_OXIDE_ENABLED" = "1" ]; then
	# Next check if Oxide doesn't' exist, or if we want to always update it
	INSTALL_OXIDE="0"
	if [ ! -f "/home/rust/steamcmd/rust/CSharpCompiler.x86_x64" ]; then
		INSTALL_OXIDE="1"
	fi
	...
	if [ "$INSTALL_OXIDE" = "1" ]; then
		...
		curl -sL $OXIDE_URL | bsdtar -xvf- -C /home/rust/steamcmd/rust/
		chmod 755 /home/rust/steamcmd/rust/CSharpCompiler.x86_x64 2>&1 /dev/null
...
if [ -f "/home/rust/steamcmd/rust/seed_override" ]; then
	RUST_SEED_OVERRIDE=`cat /home/rust/steamcmd/rust/seed_override`
	...
	if [ ! -d "/home/rust/steamcmd/rust/server/$RUST_SEED_OVERRIDE" ]; then
		echo "Creating seed override identity directory.."
		mkdir -p "/home/rust/steamcmd/rust/server/$RUST_SEED_OVERRIDE"
		if [ -f "/home/rust/steamcmd/rust/UserPersistence.db.bak" ]; then
			echo "Copying blueprint backup in place.."
			cp -fr "/home/rust/steamcmd/rust/UserPersistence.db.bak" "/home/rust/steamcmd/rust/server/$RUST_SEED_OVERRIDE/UserPersistence.db"
		fi
		if [ -f "/home/rust/steamcmd/rust/xp.db.bak" ]; then
			echo "Copying blueprint backup in place.."
			cp -fr "/home/rust/steamcmd/rust/xp.db.bak" "/home/rust/steamcmd/rust/server/$RUST_SEED_OVERRIDE/xp.db"
		fi
	fi
fi
...
if [ "$LOGROTATE_ENABLED" = "1" ]; then
	...
	if [ ! -d "/home/rust/steamcmd/rust/logs/archive" ]; then
		mkdir -p /home/rust/steamcmd/rust/logs/archive
	fi

	# Set the logfile filename/path
	DATE=`date '+%Y-%m-%d_%H-%M-%S'`
	RUST_SERVER_LOG_FILE="/home/rust/steamcmd/rust/logs/$RUST_SERVER_IDENTITY"_"$DATE.txt"

	# Archive old logs
	echo "Cleaning up old logs.."
	mv /home/rust/steamcmd/rust/logs/*.txt /home/rust/steamcmd/rust/logs/archive
else
